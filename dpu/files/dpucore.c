...
#endif
		dprint(PLEVEL_ERR, "\n");
		dprint(PLEVEL_ERR, "%-8s\t: 0x%.8x\n", "DBG_LCNT_START", pdpureg->ctlreg[i].lstart_cnt);
		dprint(PLEVEL_ERR, "%-8s\t: 0x%.8x\n", "DBG_LCNT_END", pdpureg->ctlreg[i].lend_cnt);
		dprint(PLEVEL_ERR, "%-8s\t: 0x%.8x\n", "DBG_CCNT_START", pdpureg->ctlreg[i].cstart_cnt);
		dprint(PLEVEL_ERR, "%-8s\t: 0x%.8x\n", "DBG_CCNT_END", pdpureg->ctlreg[i].cend_cnt);
		dprint(PLEVEL_ERR, "%-8s\t: 0x%.8x\n", "DBG_SCNT_START", pdpureg->ctlreg[i].sstart_cnt);
		dprint(PLEVEL_ERR, "%-8s\t: 0x%.8x\n", "DBG_SCNT_END", pdpureg->ctlreg[i].send_cnt);
		dprint(PLEVEL_ERR, "%-8s\t: 0x%.8x\n", "DBG_MCNT_START", pdpureg->ctlreg[i].pstart_cnt);
		dprint(PLEVEL_ERR, "%-8s\t: 0x%.8x\n", "DBG_MCNT_END", pdpureg->ctlreg[i].pend_cnt);
		dprint(PLEVEL_ERR, "%-8s\t: 0x%.8x\n", "AXI_STATUS", pdpureg->ctlreg[i].axi_status);
		dprint(PLEVEL_ERR, "%-8s\t: 0x%.8x\n", "TIMER_STATUS", pdpureg->ctlreg[i].timer_status);
		dprint(PLEVEL_ERR, "\n");
	}
...
// record the start time
	...
	ret = wait_event_interruptible_timeout(dpudev.waitqueue[dpu_core],
					       dpudev.intflg[dpu_core] == TRUE, timeout * CONFIG_HZ);
